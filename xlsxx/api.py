...
from xlsxx.proxy.sheet import ReadingCells
...
def read_xlsx_vertical(path, start, tailcolumn, tailrow=-1, *, sheet=None, sequential=False, nofetch=False):
    """
    シートから縦方向に値を読みだす
    Params:
        path(Path): 対象ファイル
    ...
    return read_sheet_rows(sht, start, tailcolumn, tailrow, ReadingCells(sequential=sequential, nofetch=nofetch))
